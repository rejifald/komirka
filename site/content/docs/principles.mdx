...
> **Canonical copy:** this page mirrors [`principles.md`](https://github.com/rejifald/tessellum/blob/main/principles.md) at the repo root. If they ever disagree, the repo file wins.
...
| Supply chain: dependency-declared tesserae entering the client bake | App attestation + deny-by-default source allowlist (never trust package-declared `exposure` alone) |
...
- Stale-while-revalidate for async reads: not in v1 (expired reads block on a single-flight
  re-read, bounded by `readTimeoutMs`, so `maxStalenessMs` stays honest); only ever as a
  loudly-named opt-in.
- `cfg.update(tessera, fn, { retries })` read-modify-write helper: deferred — it is a
...
- ~~Final naming~~ — decided: the package is `tessellum` (see header). Still open: whether
  the primitive stays `tessera` (with `$tessera` brand and `tesserav1|` identity prefix) or takes a
