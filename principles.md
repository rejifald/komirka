...
- **P13 — Bake is the only channel that *inlines* client config.** Build-constant
  ...
  (ADR 0008): the deploy environment injects a public JSON document into the page before boot,
  read synchronously and bound at the composition root — never inlined, always an untrusted input
  boundary revalidated on read, and structurally incapable of carrying secrets (P7). Narrowing,
  not weakening — attestation, deny-by-default client targets, and the secret-name bake gate are
  unchanged.
