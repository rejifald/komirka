...
> grain, mote, weft, dial, setpoint, filament, quark) was taken on npm. The 0.0.0
...
> **`zerno`** (зерно — Ukrainian for a grain: one small unit of config; §6) — greppable and
> on brand, free of the tuning/reactivity that `knob` and `atom` would imply.
...
- **P1 — The descriptor is inert data.** Constructing a config bit performs zero I/O, zero
  ...
  structural brand field (`$zerno`), never `instanceof` (dual-package hazard).
...
- **P8 — Eager validation over the explicitly bound subset is the default.**
  `bind(zernos, sources)` validates exactly the listed set, now, with one aggregated error.
...
- **P11 — Content-addressed identity with a frozen field list.** Identity =
  ...
  canonicalized as `zernov1|` + RFC 8785 JSON, hashed with xxh128. The hash is an index, not
  ...
  discriminants by identity, and a live twin of a snapshot zerno must not be identity-equal.
...
- **P16 — "If it can change while the process runs, it's state, not config."** The only
  exception is a zerno explicitly declared `freshness: "live"` over a *re-readable* source
...
- **P18 — Conditional structure is a relation, not a mutable descriptor.** Requiredness
  that depends on another zerno (`EMAIL_SERVICE_TYPE` selecting smtp vs sendgrid, a
  ...
  v1. Discriminants and `when()` condition zernos are `snapshot`-only and eagerly resolved
...
- **P19 — Writes are operator-plane, input-typed, and write-through.** `set()` takes the
  ...
  **context-free reads** (a `unwrap()` that takes per-request/per-user context is a flag SDK
  — refuse), **uniform** (one value per process per zerno), **operator-plane writes**
...
- **P20 — Wiring is deployment reality, never contract.** A zerno's declared refs are the
  library's contract (identity); *where that lives on this platform* is the deployment's
  wiring, spelled inline per entry (`env(zerno, "NAME")` renames a declared ref in place,
  ...
  combinators are **pure JSON-representable data** (zerno identity ref + ref kind +
  ...
  every error message (effective name first, declared in parens). A non-secret zerno may
  ...
  zerno ever (`DuplicateWiringError` — no precedence hierarchy); wiring soundness checks
...
- **P21 — Secrets get a provider class, and the class has teeth.** Provider-sourced
  ...
  **routing** (bidirectional pairing — a secret-class provider serves only secret zernos;
  provider-sourced secret zernos require a secret-class store or an explicit, lockfile-
  ...
  client's connection error can embed the provider's own credential, which no zerno-value
  ...
  **conformance profile** (poisoned-fake probes; `fakeSecretProvider()` ships
  ...
  enforcement), segregates batches by exposure, disables echo-verify for secret zernos
  everywhere, gates `literal()` on secret zernos (`SecretLiteralError` outside tests), and
...
- **The monolith through the import graph.** A grouping construct that holds references to
  ...
  central all-zernos module imported by runtime code; enumeration is bake's job (build-time),
...
- **Boot-validated green, invalid at 3am.** A `live` zerno re-validates when its raw value
  changes; an operator typo can make reads fail on paths that boot-validated green.
  *Decision (author):* **current truth wins** — no magically inherited values. The zerno
  ...
  fires once per (zerno, distinct invalid raw), receiving `{ rawDigest, length }` for
  secrets. Keep-last-good is a **bind-site opt-in** —
  `{ invalid: [keepLastGood(zerno, { maxCoastMs })] }`, `maxCoastMs` required, past budget
...
- **One bad zerno must not 500 the whole site.** All-or-nothing pins would turn one invalid
  *unused* zerno into a 100% outage at the per-request pin. *Rule:* `snapshot()` never
  throws wholesale — an invalid zerno occupies an **error slot**; only `unwrap()` of that zerno
...
- **Invalid is not unavailable.** A provider that answered with a bad value and a provider
  ...
  = min(app, config DB).
- **Health probes amplify config failures.** "Any live zerno invalid" wired into readiness
  ...
  *Rule:* `health()` distinguishes `valid | invalid-was-valid | never-valid | coasting`
  ...
  operator's triage split); documented probe contract: liveness never consults zerno
...
- **Async validators detonating in sync paths.** Standard Schema's `validate()` may legally
  ...
  checks for a thenable and throws `AsyncValidatorInSyncBindingError` naming the zerno and
...
- **`.values` is an aggregate leak surface.** One `logger.info({ cfg: cfg.values })`
  ...
  documented as "`unwrap()` on every zerno"; record keys `toJSON`/`then`/`constructor` are
...
- **Type widening must fail safe, not freeze silently.** A `Zerno<string>` annotation
...
- **A rename that silently replaces is an outage deferred.** If `env(zerno, "CUSTOM")`
  replaced the declared ref *set*, a deployment that only meant to rename would silently
  drop the file fallback the zerno's author declared for outages. *Rule:*
  `env`/`file`/`provider(zerno, …)` rename the matching declared ref in place; `chain()`
...
- **"Live" env vars are physically impossible on Node.** The OS environment block of a
  ...
  zerno works in unit tests (which mutate `process.env`) and silently does nothing in
...
- **Torn config.** Two reads of a live pair (DB host + password rotated together) inside
  ...
  pass (one file read serves all zernos from that file).
- **Liveness poisoning the DI contract.** A library typed against `Snapshot` must be able
  ...
  any live zerno is a `LiveBinding`, not assignable where `Snapshot` is expected; `pick()`
  pins.
- **Workers global-state trap.** Module-scope binding is impossible on workerd (no env
  ...
  `(zerno identity hash, rawString)`.
- **The syscall amplifier.** Live file sources re-read on every `unwrap()` — 3 zernos × 2k req/s
  = 6k synchronous reads/s on the event loop. *Rule:* poll-on-read is throttled per source
  (`maxStalenessMs`, default ~1s for files); one read per window serves all zernos fed by
  that source.
...
- **Live buys nothing for constructed values.** `new Pool(cfg.unwrap(dbUrl))` at boot never
  ...
  docs (with the Pool example); ship `binding.epoch(zerno)` — a monotonic counter — so user
  code rebuilds derived state cheaply; no watch/subscribe API in core.

### Identity & cross-package DI

- **Schema-swap interchangeability.** The validator cannot be hashed, so two same-key zernos
...
- **Cosmetics in the identity hash.** Hashing `doc` makes two semver-distinct installs
  ...
  identity field list is closed and versioned (`zernov1|`); everything else — including
...
- **Collisions that never meet.** Two bindings created in different modules never pass
  ...
  `unwrap()` has a three-way contract — hit / `UnboundZernoError` /
...
- **Dual-package canonicalization drift.** The config library itself duplicated at two
  ...
  authoritative and the hash is only an index; `$zerno` brand, never `instanceof`.
...
- **Import-time source spoofing.** A mutable global `provide()` lets any dependency
  redirect a zerno's source at import time. *Rule:* there is no global provide; providers
...
- **`unwrap()` precedence must never suggest `unify()` across sibling branches.** Same-key
  ...
  member → `InactiveVariantError` (a *subclass* of `UnboundZernoError`, so the three-way
  contract is refined, not broken) → key collision vs direct/*active* zernos only →
  `DescriptorMismatchError` → `UnboundZernoError`.
- **A live discriminant is torn topology.** A branch switch mid-process means half a
  request on smtp, half on sendgrid. *Rule:* discriminants and `when()` condition zernos
...
- **Branch zernos bound directly are contradictory membership.** `bind([email, smtp.host])`
  claims smtp.host is both always-required and branch-conditional. *Rule:*
  `AmbiguousZernoMembershipError` at bind.
...
- **Defaults must not resurrect a dead subtree.** A condition zerno living in an inactive
...
- **Multi-zerno writes tear durable state.** A crash between the discriminant write and its
...
- **Write-time validation is writer-local.** A v1 replica's `set()` can write a value v2
  ...
  `health()` + per-zerno `keepLastGood` recommended for DB-sourced zernos.
- **Hot-zerno expiry is a thundering herd.** 500 concurrent requests one ms after staleness
...
- **The write path re-opens secret leaks.** `onInvalid(zerno, raw, …)` pipes raw secrets to
  log hooks; validator and DB-driver messages echo values; `set()` rethrows provider
  errors to admin UIs. *Rule:* structural redaction for secret zernos — hooks receive
...
- **Byte-equality verification dies on normalizing stores.** JSONB reorders keys; triggers
  ...
  no fixpoint. *Rule:* equality at the schema level (byte-equal OR parsed-equal), per-zerno
...
- **Auto-revert of a secret un-rotates a leaked credential.** The store mangles the *new*
  ...
  about truncation. *Rule:* secret zernos refuse `verify: "revert"` unless opted in
  per-zerno; `WriteMangledError` always states `nowLive: "prev" | "unknown"` with the
...
- **Bake executes the manifest.** Evaluating a user module graph at build time is the
  ...
  the manifest must be side-effect-free (checked: zerno construction is inert, so a manifest
...
- **The spread hides the attestation delta.** With a compose-only manifest
  (`client: [...emailPublic]`), a dependency bump that adds a zerno to `emailPublic` leaves
  the manifest byte-identical — CODEOWNERS never fires, and the new zerno ships to the
  ...
  version and source refs, so "this zerno became DB-mutable" shows up in review).
  ...
  Feature modules own their zerno groups next to their code; zerno definitions *inside* the
...
- **The CI gate is adversary-triggerable.** `bake --check` runs on every PR and executes
  the manifest import closure — including zerno files inside dependency-bumped third-party
...
- **Glob discovery captures the wrong zernos.** Repo-root globs pull sibling apps' secrets
...
- **A variants group is one bundle unit.** Importing the group retains *all* branch
  ...
  baked clients are immune (shim only), enforced by the client-graph lint (no `*.zernos.ts`
...
| Secrets echoed by validator error messages | For secret zernos, BYO validator text is untrusted: reports carry issue counts and paths only, never messages or values |
| Secrets in CI logs via the scanner itself | Scan findings report location + zerno key with scrubbed context — never the value; bake never holds secret values at all (P14) |
...
catches hand-rolled secrets that never became zernos, and catches accidental `define`
...
- **No runtime registry, no derived-zerno expression language, no reachability-analysis
